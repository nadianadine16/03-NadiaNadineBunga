...
<?php if($hasilSearchSurat==NULL){?>
  <b>Data tidak ditemukan</b>  
  <?php
    } 
    else { ?>
    <?php $no=1; foreach($hasilSearchSurat as $hss):?>
  <tr style="color:black">
    <td><?=$hss["nomor_surat"];?></td>
    <td><?=$hss["nama_kategori"];?></td>
    <td><?=$hss["judul"];?></td>
    <td><?=$hss["waktu_pengarsipan"];?></td>
    <td><a onclick="return konfirmasi()" href="<?= base_url();?>Surat/Hapus_Data_Surat/<?=$hss['id_surat'];?>" class="btn btn-danger" style="margin-right:2px;">Hapus</a>
        <a href="<?= base_url('upload/'.$hss["file"])?>" class="btn btn-warning" style="margin-right:2px;">Unduh</a>
        <a href="<?= base_url();?>Surat/Lihat_Surat/<?=$hss['id_surat'];?>" class="btn btn-primary">Lihat >></a></td>
  </tr>  
  <?php endforeach;?>  
    <?php
    } 
